...
int main(short int argc, char **argv) {
  ...
  if (DBUG == 1 ) printf("ptrs.fwd_inv = 0x%x\n",ptrs.fwd_inv);
		...
		if (tmp == 0) { 
			...
			else fread(ptrs.inp_buf,sizeof(short int),4096,inptr);
			//else fread(ptrs.inpbuf,sizeof(short int),65536,inptr);
			...
			else fread(ptrs.inpbuf,sizeof(short int),4096,inptr);
			//else fread(ptrs.inpbuf,sizeof(short int),65536,inptr);
			fclose(inptr);
	...
	fwrite(ptrs.inp_buf,sizeof( short int),4096,outptr);
